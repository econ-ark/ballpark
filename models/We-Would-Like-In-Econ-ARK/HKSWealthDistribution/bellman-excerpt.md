# HKS (2018) — Bellman Excerpt (Formalization Entry Point)
...
2. **Function-valued objects $\tau(\cdot), r^X(\cdot), \sigma^X(\cdot), \psi(\cdot)$.** The paper uses piecewise and tabulated schedules (11-bracket tax, 13-bin portfolio schedule, Pareto-tail labour-efficiency map), not parametric closed forms. Flag: `# unresolved:` — needs either a lookup-table idiom (not in the current dolo-plus spec) or an explicit parametric approximation.
...
4. **EGM feasibility under wealth-dependent returns.** Carroll-style EGM uses an envelope condition that is clean only when the gross-return factor is independent of $a$. Here $r^X(a), \sigma^X(a)$ break that; the Euler inversion is not obviously well-posed as written. Flag: `# unresolved:` in the InvEuler block (first-pass YAML is likely to omit InvEuler/MarginalBellman and rely on direct Bellman iteration).
5. **Inter-stage and inter-period value-function wiring.** Mechanically present (Section 4), but each connector needs an explicit YAML declaration: consumption.continuation ↔ shock.arrival, and shock.continuation ↔ next-period consumption.arrival. Flag: `# unresolved:` — awaiting a dolo-plus canonical example of two-stage composition.
